...
async function getConversionPathInsight(accountId: string): Promise<ConversionPathResult> {
  ...
  try {
    ...
    let pathTransitionCount = 0;
    let pathCompletionCount = 0;
    ...
    for (let step = 0; step < MAX_STEPS; step++) {
      ...
      const nextEdgeCount = Number(next.count);

      if (step === 0) {
        pathTransitionCount = nextEdgeCount;
      }
      pathCompletionCount =
        pathCompletionCount === 0 ? nextEdgeCount : Math.min(pathCompletionCount, nextEdgeCount);
...
export default async function HomePage() {
  ...
  const totalDropoffCount = dropoffData.candidates.reduce((sum, item) => sum + item.incomingCount, 0);
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <DropoffInsightCard
            ...
            totalDropoffCount={totalDropoffCount}
